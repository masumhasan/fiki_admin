...
export default function QuotationPage({
  ...
  const handleSendQuote = async () => {
    ...
    if (!token) {
      setNotice({ text: "Not authenticated. Please log in again.", ok: false });
      return;
    }
    setSending(true);
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1fr)_280px]">
          <div className="space-y-4">
              ...
              <div className="grid gap-4 p-5 md:grid-cols-3">
                ...
                <div className="md:col-span-3 rounded-xl border border-[#cdddf6] bg-[#eef5ff] p-5">
                  ...
                  <div className="mt-5 grid gap-3 text-sm text-[#71829b] md:grid-cols-2">
                    <div>
                      <p>Base Fare <b className="float-right text-[#172033]">{money(base)}</b></p>
                      <p className="mt-3">Distance Cost ({miles} mi × ${rate.toFixed(2)}) <b className="float-right text-[#172033]">{money(miles * rate)}</b></p>
                      <p className="mt-3">Extra Services <b className="float-right text-[#172033]">{money(assist)}</b></p>
                    </div>
                    <div>
            ...
            <section className="overflow-hidden rounded-xl border border-[#e1e6ee] bg-white shadow-[0_4px_14px_rgba(15,37,74,.04)]">
              <div className="flex items-center gap-2 border-b px-5 py-3">
                ...
                <h2 className="text-sm font-bold text-[#2a3b54]">Note to Passenger</h2>
              </div>
              <div className="p-5">
                <textarea
                  className="h-24 w-full rounded-lg border border-[#dce5f0] bg-white px-3 py-2 text-[13px] font-medium text-[#27344a] outline-none focus:border-[#173d76] focus:ring-4 focus:ring-[#173d76]/10 resize-none"
                  placeholder="Optional note to the passenger about this quotation…"
                  value={quoteNote}
          ...
          <aside className="space-y-6">
            ...
            <article className="rounded-xl bg-[#173d76] p-5 text-white shadow-[0_4px_14px_rgba(15,37,74,.12)]">
              ...
              <p className="mt-2 text-sm text-blue-100">Auto-calculated · updates in real-time</p>
  ...
  return (
    <label className="text-[12px] font-semibold text-[#58677d]">
      ...
      <span className="relative mt-2 block">
        <span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[#8493a8]">{prefix}</span>
        <input
