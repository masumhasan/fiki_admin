    ...
    try {
      ...
      if (res.success && res.data && res.data.drivers) {
        ...
        const mapped: Driver[] = res.data.drivers
          ...
          const vehicleStr = [vehicleMake, vehicleModel].filter(Boolean).join(" ") || "—";
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-4 lg:gap-5">
        <Summary label="Total drivers" value={loading ? null : drivers.length} tone="text-primary" />
        <Summary
          label="Active now"
          value={loading ? null : drivers.filter((d) => d.status === "Active").length}
          tone="text-emerald-500"
        />
        <Summary
          label="On trip"
          value={loading ? null : drivers.filter((d) => d.status === "On trip").length}
          tone="text-blue-500"
        />
        <Summary
          label="Off duty"
          value={loading ? null : drivers.filter((d) => d.status === "Off duty").length}
...
function Summary({
  ...
  return (
    <article className="rounded-xl border border-[#e1e6ee] bg-card p-5 shadow-[0_4px_14px_rgba(15,37,74,.04)]">
      ...
      <p className={`mt-2 text-4xl font-bold leading-none tracking-[-.04em] ${tone}`}>
        {value === null ? (
  ...
  return (
    <article className="rounded-xl border border-[#e1e6ee] bg-card p-5 shadow-[0_4px_14px_rgba(15,37,74,.04)]">
      ...
      <dl className="mt-5 grid grid-cols-2 gap-x-4 gap-y-4 text-xs">
        ...
        <DriverInfo icon={Activity} label="Trips" value={`${driver.trips} this week`} />
      </dl>
...
function DriverInfo({
  ...
  return (
    <div>
      ...
      <dd className="mt-1.5 truncate font-semibold text-foreground" title={value}>
        {value}
      </dd>
    </div>
  );
...
function DriverStatus({ status }: { status: DriverStatus }) {
  ...
  return (
    <span className={`rounded-full px-3 py-1 text-[10px] font-bold ${style}`}>{status}</span>
  );
}
                ...
                {visible.map((item) => (
                  ...
                  >
                    <td className="px-6 py-4">
                      ...
                      <span className="text-[10px] text-brand-placeholder">{item.id}</span>
                    </td>
                    <td>
                    ...
                    <td>
                      <span className="font-medium text-foreground">Driver — {item.type}</span>
              ...
              <article className="p-5" key={item.id}>
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-[10px] text-brand-placeholder">{item.id}</p>
                  </div>
                  <ApplicationBadge value={item.status} />
                </div>
